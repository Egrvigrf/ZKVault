...
本轮提交继续推进“阶段二：终端接入层标准化”。在上一轮已经细化输入阶段和失败恢复语义之后，这次继续把列表视图本身组织成可复用的浏览上下文，让 shell 原型开始具备更接近未来 TUI 的“列表焦点 -> 详情查看”编排方式。

- 在 `src/app/frontend_contract.*` 中新增带焦点的列表结果载荷与渲染规则，并把 `next` / `prev` / `show [name]` 纳入共享命令契约
- `src/shell/interactive_shell.cpp` 开始持有当前筛选词、可见条目集和选中项；`list` / `find` 建立浏览上下文，`next` / `prev` 移动焦点，`show` 无参时直接读取当前选中项
- 会话 `lock` 时会清空浏览上下文，避免解锁后继续复用旧的列表焦点；显式按名称 `show <name>` 时会重新建立对应条目的浏览定位
- 扩展 `zkvault_frontend_contract` 与 `zkvault_shell_smoke`，覆盖焦点渲染、导航移动、无参详情查看以及锁屏后焦点失效等回归场景

这次改动后，终端前端已经不只是暴露“能列出名字”和“能显示详情”这两个离散动作，而是开始具备列表视图与详情视图之间的共享状态承接关系，为后续全屏 TUI 接入焦点管理和局部刷新打基础。
