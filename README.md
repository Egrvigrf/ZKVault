...
- `tui` 当前已切换为按键驱动：可用 `Up` / `Down` 或 `j` / `k` 移动列表焦点，按 `Enter` 查看详情，按 `a` 打开新增表单，按 `d` 打开删除确认，按 `?` 打开帮助，按 `Esc` 返回浏览区或取消当前表单，按 `l` / `u` 锁定或解锁，按 `q` 退出
- `tui` 当前已覆盖浏览、帮助、详情、锁定、条目新增与条目删除；条目更新和主密码轮换仍建议通过 CLI 或 `shell` 原型完成
...
本轮提交继续推进“阶段三：TUI 应用实现”。在上一轮已经把 `zkvault tui` 从“显示上一条命令输出”推进为显式视图编排和按键驱动浏览之后，这次进一步把新增、删除两类写操作真正收进全屏界面，而不是退回 CLI 或 `shell` 提示符。

- `src/shell/shell_runtime.cpp` 抽出可复用的写操作辅助函数，让 TUI 可以直接复用已有会话状态、存储逻辑和删除逻辑，而不是复制一套 CLI 分支
- `src/tui/terminal_ui.cpp` 现在显式维护新增表单态和删除确认态，支持在全屏界面内输入名称、密码、备注，并在删除前要求再次键入条目名
- `tui` 的快捷键解释被限制在非模态视图中，避免 `a`、`d`、`j`、`k` 这类键在表单输入时被错误吞掉
- 扩展 `zkvault_tui_smoke`，覆盖 TUI 内新增条目、取消删除、确认删除、详情查看、帮助页、锁定后的解锁视图以及备用屏幕退出，固化当前这条“最小可写真 TUI”路径

这次改动后，全屏 TUI 已经不再只是“列表焦点 + 快捷键导航 + 详情切换”的骨架，而是开始具备真正的写操作闭环。后续继续补充更新表单、主密码轮换和更多快捷键时，可以直接沿着这套按键驱动循环与共享状态机继续扩展。
...
- 补充界面层尚未完成的更新表单、主密码轮换、更多快捷键和局部刷新机制
